...
Du code tourne légitimement sur une seule machine : tout ce qui est derrière `GetLocalPlayer()`, la souris et les clics async, le slide async d'un héros sur sa propre machine, la caméra, l'interface. Ce code peut **montrer** des choses différentes sur chaque machine, mais ne doit jamais **changer** ce qu'est la partie.
...
- **Frames, agents depuis Warcraft III 3.0** (`type framehandle extends agent` dans son `common.j` ; simples handles avant) : une frame créée ou détruite par une seule machine, sans conséquence jusque-là, désynchronise maintenant la partie. La souris async était lue par une grille de centaines de frames, et le clic local par 1 ou 3 boutons de capture selon la largeur de l'écran de chaque machine : les deux ont disparu, remplacés par les natives de la 3.0 qui lisent la souris (`BlzGetMouseScreenPosX/Y`, `BlzIsMouseButtonPressed`, dans `src/core/Async_slide/AsyncMouse.ts` et `AsyncSlideInput.ts`), sans aucune frame. L'historique de commandes de `src/App/Interface.ts` reste désactivé pour avoir désynchronisé.
