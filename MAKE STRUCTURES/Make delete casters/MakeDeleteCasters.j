//TESH.scrollpos=29
//TESH.alwaysfold=0
library MakeDeleteCasters needs Make
...
struct MakeDeleteCasters extends Make
    ...
    static method create takes unit maker, string mode returns MakeDeleteCasters
        //modes : oneByOne, twoClics
	    local MakeDeleteCasters m
		if (maker == null or (mode != "oneByOne" and mode != "twoClics")) then
			return 0
		endif
        set m = MakeDeleteCasters.allocate()
        set m.maker = maker
		set m.makerOwner = GetOwningPlayer(maker)
        set m.kind = "deleteCasters"
        set m.mode = mode
