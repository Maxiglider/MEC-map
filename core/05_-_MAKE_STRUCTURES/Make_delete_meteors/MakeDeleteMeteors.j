//TESH.scrollpos=49
//TESH.alwaysfold=0
library MakeDeleteMeteors needs Make
...
struct MakeDeleteMeteors extends Make
    ...
    static method create takes unit maker, string mode returns MakeDeleteMeteors
        //modes : oneByOne, twoClics
	    local MakeDeleteMeteors m
		if (maker == null or (mode != "oneByOne" and mode != "twoClics")) then
			return 0
		endif
        set m = MakeDeleteMeteors.allocate()
        set m.maker = maker
		set m.makerOwner = GetOwningPlayer(maker)
        set m.kind = "deleteMeteors"
        set m.mode = mode
        ...
        if (mode == "oneByOne") then
            call TriggerRegisterUnitEvent(m.t, m.maker, EVENT_UNIT_ISSUED_TARGET_ORDER)
        endif
        return m
	endmethod
