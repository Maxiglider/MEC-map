//TESH.scrollpos=0
//TESH.alwaysfold=0
library MonsterTeleportArray needs MonsterTeleport


struct MonsterTeleportArray [25000] //50 niveaux * 500 monstres
    private MonsterTeleport array monsters [MAX_NB_MONSTERS]
	private integer lastInstance
	
	static method create takes nothing returns MonsterTeleportArray
    ...
    method get takes integer arrayId returns MonsterTeleport
    ...
    method new takes MonsterType mt, real period, real angle, string mode, boolean createUnit returns MonsterTeleport
		local integer n        
		...
		//if (n > .lastInstance) then
			set .lastInstance = n
		//endif
		set .monsters[n] = MonsterTeleport.create(mt, period, angle, mode)
        if (createUnit) then
            call .monsters[n].createUnit()
        endif
        set .monsters[n].level = udg_levels.getLevelFromMonsterTeleportArray(this)
    ...
    method clearMonster takes integer monsterId returns boolean
        local integer i = 0
        loop
            exitwhen (.monsters[i] == MonsterTeleport(monsterId) or i > .lastInstance)
            set i = i + 1
        endloop
    ...
    method getMonsterNear takes real x, real y returns MonsterTeleport
        local real xMob
        local real yMob
        local integer i = 0
        loop
