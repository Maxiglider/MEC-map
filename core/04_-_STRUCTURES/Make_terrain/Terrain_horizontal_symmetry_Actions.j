//TESH.scrollpos=0
//TESH.alwaysfold=0
library TerrainHorizontalSymmetryActions needs Escaper


function TerrainHorizontalSymmetry_Actions takes nothing returns nothing
    ...
    if (mk.isLastLocSavedUsed()) then
        set action = MakeTerrainHorizontalSymmetryAction.create(mk.lastX, mk.lastY, x, y)
        if (action == 0) then
