//TESH.scrollpos=38
//TESH.alwaysfold=0
library MakeVisibilityModifier needs Make
...
struct MakeVisibilityModifier extends Make
	...
	static method create takes unit maker returns MakeVisibilityModifier
        local MakeVisibilityModifier m
		if (maker == null) then
			return 0
		endif
        set m = MakeVisibilityModifier.allocate()
        set m.maker = maker
		set m.makerOwner = GetOwningPlayer(maker)
        set m.kind = "visibilityModifierCreate"
