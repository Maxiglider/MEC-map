//TESH.scrollpos=18
//TESH.alwaysfold=0
library MakeTerrainVerticalSymmetry needs Make, BasicFunctions, ModifyTerrainFunctions



struct MakeTerrainVerticalSymmetry extends Make    
    real lastX
    real lastY
	...
	static method create takes unit maker returns MakeTerrainVerticalSymmetry
        local MakeTerrainVerticalSymmetry m
		if (maker == null) then
			return 0
		endif
        set m = MakeTerrainVerticalSymmetry.allocate()
        set m.maker = maker
		set m.makerOwner = GetOwningPlayer(maker)
        set m.kind = "terrainVerticalSymmetry"
