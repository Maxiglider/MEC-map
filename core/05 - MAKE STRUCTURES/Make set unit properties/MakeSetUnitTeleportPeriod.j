//TESH.scrollpos=0
//TESH.alwaysfold=0
library MakeSetUnitTeleportPeriod needs Make
...
struct MakeSetUnitTeleportPeriod extends Make
    ...
    static method create takes unit maker, string mode, real period returns MakeSetUnitTeleportPeriod
        //modes : oneByOne, twoClics
	    local MakeSetUnitTeleportPeriod m
		if (maker == null or (mode != "oneByOne" and mode != "twoClics") or period < MONSTER_TELEPORT_PERIOD_MIN or period > MONSTER_TELEPORT_PERIOD_MAX) then
			return 0
		endif
        set m = MakeSetUnitTeleportPeriod.allocate()
        set m.maker = maker
		set m.makerOwner = GetOwningPlayer(maker)
        set m.kind = "setUnitTeleportPeriod"
        set m.mode = mode
        set m.period = period
