const initMMSimplePatrolActions = () => {
    // needs TerrainTypeFunctions, Escaper
    ...
    const MonsterMakingSimplePatrol_Actions = () => {
        ...
        let mk: MakeMonsterSimplePatrol = MakeMonsterSimplePatrol(integer(mkGeneral))
        ...
        if (mk.getMode() == 'normal') {
            if (mk.isLastLocSavedUsed()) {
                if (BasicFunctions.GetLocDist(mk.lastX, mk.lastY, x, y) <= PATROL_DISTANCE_MIN) {
                    ...
                    monster = escaper
                        .getMakingLevel()
                        .monstersSimplePatrol.new(mk.getMonsterType(), mk.lastX, mk.lastY, x, y, true)
