...
const initSaveLevels = () => {
    ...
    const SaveMonstersMultiplePatrols_Actions = () => {
        ...
        while (true) {
            ...
            if (level.monstersMultiplePatrols.get(monsterMultiplePatrolsId) != 0) {
                StringArrayForCache.stringArrayForCache.push(
                    level.monstersMultiplePatrols.get(monsterMultiplePatrolsId).toString()
                )
            }
    ...
    const SaveMonstersSimplePatrol_Actions = () => {
        ...
        while (true) {
            ...
            if (level.monstersSimplePatrol.get(monsterSimplePatrolId) != 0) {
                StringArrayForCache.stringArrayForCache.push(
                    level.monstersSimplePatrol.get(monsterSimplePatrolId).toString()
                )
            }
    ...
    const StartSaveLevel = () => {
        ...
        if (level === null) {
            ...
            //start message
            if (level.getStartMessage() != null && level.getStartMessage() != '') {
                StringArrayForCache.stringArrayForCache = new StringArrayForCache(
                    'level' + I2S(levelId),
                    'startMessage',
                    false
