const initSaveTerrainRamps = () => { // needs Text, SaveWater
...
const SaveTerrainRamps_Actions = (): void => {
			...
			if ((!IsNearBounds(x, y))) {
				...
				if ((!isRampMiddleRaisedB)) {
					...
					if ((otherX <= MAP_MAX_X)) {
						...
						walkable = !IsTerrainPathable(otherX, otherY + DECAL_TEST_PATH, PATHING_TYPE_WALKABILITY) || !IsTerrainPathable(otherX, otherY - DECAL_TEST_PATH, PATHING_TYPE_WALKABILITY);
						...
						if ((currentCliffLevel !== otherCliffLevel)) {
							...
							if ((walkable)) {
								...
								walkable2 = !IsTerrainPathable(otherX, otherY + DECAL_TEST_PATH, PATHING_TYPE_WALKABILITY) || !IsTerrainPathable(otherX, otherY - DECAL_TEST_PATH, PATHING_TYPE_WALKABILITY);
								if ((!walkable2)) {
									...
									if ((GetLocationZ(loc1) !== GetLocationZ(loc2) && GetTerrainCliffLevelBJ(loc1) === GetTerrainCliffLevelBJ(loc2))) {
										ramp = true;
									}
									RemoveLocation(loc1)
									RemoveLocation(loc2)
				...
				if ((!isRampMiddleRaisedB)) {
					...
					if ((otherY <= MAP_MAX_Y)) {
						...
						walkable = !IsTerrainPathable(otherX + DECAL_TEST_PATH, otherY, PATHING_TYPE_WALKABILITY) || !IsTerrainPathable(otherX - DECAL_TEST_PATH, otherY, PATHING_TYPE_WALKABILITY);
						...
						if ((currentCliffLevel !== otherCliffLevel)) {
							...
							if ((walkable)) {
								...
								walkable2 = !IsTerrainPathable(otherX + DECAL_TEST_PATH, otherY, PATHING_TYPE_WALKABILITY) || !IsTerrainPathable(otherX - DECAL_TEST_PATH, otherY, PATHING_TYPE_WALKABILITY);
								if ((!walkable2)) {
									//il y a une rampe à condition que les deux points en bas de la falaise soient au même niveau de falaise et pas à la même hauteur
									if ((currentCliffLevel < otherCliffLevel)) {
										loc1 = Location(x, y);
 ...
 stringArrayForCache.push(rampStr)
			x = x + LARGEUR_CASE;
		}
		y = y + LARGEUR_CASE;
		loc1 = null;
...
const StartSaveTerrainRamps = (): void => {
	y = MAP_MIN_Y;
	stringArrayForCache = StringArrayForCache.create("terrain", "terrainRamps", false)
