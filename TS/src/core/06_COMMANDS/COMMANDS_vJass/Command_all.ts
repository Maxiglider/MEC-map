...
const initCommandAll = () => {
    const ExecuteCommandAll = (escaper: Escaper, cmd: string): boolean => {
        ...
        if (name === 'cameraField' || name === 'cf') {
            if (!(nbParam === 1 && FunctionsOnNumbers.IsInteger(param1))) {
                return true
            }
        ...
        if (name === 'getTerrainInfo' || name === 'gti') {
            ...
            if (nbParam === 2) {
                if (FunctionsOnNumbers.IsInteger(param1) && FunctionsOnNumbers.IsInteger(param2)) {
                    i = S2I(param1)
                    n = S2I(param2)
                    if (
                        i >= 1 &&
        ...
        if (name === 'disco' || name === 'd') {
            if (noParam) {
                ...
                if (nbParam === 1) {
                    if (FunctionsOnNumbers.IsInteger(param1)) {
                        n = S2I(param1)
                        if (n < 1 || n > 30) {
                            return true
                        }
    ...
    return { ExecuteCommandAll }
}

export const CommandAll = initCommandAll()
