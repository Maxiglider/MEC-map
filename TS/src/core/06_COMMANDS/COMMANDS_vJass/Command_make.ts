...
const initCommandMake = () => {
    const ExecuteCommandMake = (escaper: Escaper, cmd: string): boolean => {
        ...
        if (name === 'newSlide' || name === 'news') {
            ...
            if (nbParam >= 3) {
                if (!FunctionsOnNumbers.IsInteger(param3)) {
        ...
        if (name === 'setTerrainWalkSpeed' || name === 'settws') {
            ...
            if (!FunctionsOnNumbers.IsPositiveInteger(param2) || S2R(param2) > 522) {
                Text.erP(escaper.getPlayer(), 'wrong speed value, should be a real between 0 and 522')
                return true
            }
        ...
        if (name === 'newCaster' || name === 'newc') {
            ...
            udg_casterTypes.new(
                param1,
                udg_monsterTypes.get(param2),
                udg_monsterTypes.get(param3),
                x,
                speed,
                y,
            ...
            if (x !== 0 && (x > CLEAR_MOB_MAX_DURATION || x < ClearMob_FRONT_MONTANT_DURATION)) {
                Text.erP(
                    ...
                    'the disable duration must be a real between ' +
                        R2S(ClearMob_FRONT_MONTANT_DURATION) +
                        ' and ' +
        ...
        if (name === 'getTerrainCliffClass' || name === 'gettcc') {
            ...
            Text.mkP(
                escaper.getPlayer(),
                'cliff class for that terrain is ' + I2S(udg_terrainTypes.get(param1).getCliffClassId())
            )
            return true
        }
    ...
    return { ExecuteCommandMake }
}

export const CommandMake = initCommandMake()
