...
const initHeroes = () => { // initializer Init_Heroes needs Escaper
...
const Trig_heroes_Actions = (): void => {
 ...
 udg_escapers.get(n).createHero(GetLocationX(startPositionsRandomized[n]), GetLocationY(startPositionsRandomized[n]), HERO_START_ANGLE)
		} else {
...
const Init_Heroes = (): void => {
	...
	TriggerRegisterTimerEvent(gg_trg_Heroes, spawnPeriod, false)
};
