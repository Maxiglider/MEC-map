...
const initCasterFunctions = () => { // needs EscaperFunctions
...
const CasterTryToShoot = (): void => {
 ...
 CasterShot.create(caster.getProjectileMonsterType(), x3, y3, angleDeTir, caster.getProjectileSpeed(), caster.getRange())
				}
			} else {
 ...
 TimerStart(caster.t, caster.getLoadTime(), false, function CasterTryToShoot)
 ...
 TimerStart(caster.t, ECART_CHECK, false, function CasterTryToShoot)
	}
};
...
const CasterUnitWithinRange_Actions = (): void => {
	...
	if ( (caster.canShoot) ) {
 TimerStart(caster.t, 0, false, function CasterTryToShoot)
	}
};
