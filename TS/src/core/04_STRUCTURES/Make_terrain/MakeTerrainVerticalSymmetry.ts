...
const initMakeTerrainVerticalSymmetry = () => { // needs Make, BasicFunctions, ModifyTerrainFunctions
...
const create = (maker: unit): MakeTerrainVerticalSymmetry => {
	let m: MakeTerrainVerticalSymmetry;
	if ((maker === null)) {
		return 0;
	}
	m = MakeTerrainVerticalSymmetry.allocate()
	m.maker = maker
	m.makerOwner = GetOwningPlayer(maker)
	m.kind = "terrainVerticalSymmetry"
	m.t = CreateTrigger()
