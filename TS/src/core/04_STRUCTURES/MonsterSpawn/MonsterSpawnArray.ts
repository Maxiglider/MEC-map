...
const initMonsterSpawnArray = () => { // needs MonsterSpawn


//struct MonsterSpawnArray //50 levels * 100 monster spawns

// TODO; Used to be private
     MonsterSpawn array monsterSpawns[100]
// TODO; Used to be private
...
const create = (): MonsterSpawnArray => {
...
const get = (arrayId: number): MonsterSpawn => {
...
const getFromLabel = (label: string): MonsterSpawn => {
	let i = 0;
	while (true) {
...
const new = (label: string, mt: MonsterType, sens: string, frequence: number, x1: number, y1: number, x2: number, y2: number, activate: boolean): MonsterSpawn => {
...
const clearMonsterSpawn = (label: string): boolean => {
...
const setMonsterType = (label: string, mt: MonsterType): boolean => {
	let ms = this.getFromLabel(label);
	if ((ms === 0)) {
		return false;
	}
...
// TODO; Used to be public
const setSens = (label: string, sens: string): boolean => {
	let ms = this.getFromLabel(label);
	if ((ms === 0)) {
		return false;
	}
...
// TODO; Used to be public
const setFrequence = (label: string, frequence: number): boolean => {
	let ms = this.getFromLabel(label);
	if ((ms === 0)) {
		return false;
	}
...
const changeLabel = (oldLabel: string, newLabel: string): boolean => {
