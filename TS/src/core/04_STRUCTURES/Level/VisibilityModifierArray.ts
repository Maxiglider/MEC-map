...
const initVisibilityModifierArray = () => { // needs VisibilityModifier


//struct VisibilityModifierArray //50 niveaux * 100 VM

// TODO; Used to be private
	 VisibilityModifier array vms 
// TODO; Used to be private
...
const create = (): VisibilityModifierArray => {
...
const new = (x1: number, y1: number, x2: number, y2: number): VisibilityModifier => {
...
const newFromExisting = (vm: VisibilityModifier): VisibilityModifier => {
...
const get = (visibilityId: number): VisibilityModifier => {
...
const setNull = (arrayId: number): void => {
	if ((arrayId >= 0 && arrayId <= this.lastInstance)) {
		this.vms[ arrayId ] = 0;
	}
};

const removeAllVisibilityModifiers = (): void => {
	while (true) {
...
const removeLasts = (numberOfVMToRemove: number): boolean => {
