...
const initMonsterSimplePatrolArray = () => { // needs MonsterSimplePatrol



//struct MonsterSimplePatrolArray //50 niveaux * 500 monstres

// TODO; Used to be private
     MonsterSimplePatrol array monsters [MAX_NB_MONSTERS]
// TODO; Used to be private
...
const create = (): MonsterSimplePatrolArray => {
...
const get = (arrayId: number): MonsterSimplePatrol => {
...
const new = (mt: MonsterType, x1: number, y1: number, x2: number, y2: number, createUnit: boolean): MonsterSimplePatrol => {
 ...
 this.monsters[n].createUnit()
	...
	this.monsters[n].level = udg_levels.getLevelFromMonsterSimplePatrolArray(this)
...
const clearMonster = (monsterId: number): boolean => {
	let i = 0;
	while (true) {
		if ((this.monsters[i] === MonsterSimplePatrol(monsterId) || i > this.lastInstance)) break;
		i = i + 1;
	}
...
const getMonsterNear = (x: number, y: number): MonsterSimplePatrol => {
	let xMob: number;
	let yMob: number;
	let i = 0;
	while (true) {
