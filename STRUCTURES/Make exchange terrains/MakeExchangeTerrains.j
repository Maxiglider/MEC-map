//TESH.scrollpos=39
//TESH.alwaysfold=0
library MakeExchangeTerrains needs Make, BasicFunctions, ModifyTerrainFunctions



struct MakeExchangeTerrains extends Make    
    real lastX
    real lastY
	...
	static method create takes unit maker returns MakeExchangeTerrains
        local MakeExchangeTerrains m
		if (maker == null) then
			return 0
		endif
        set m = MakeExchangeTerrains.allocate()
        set m.maker = maker
		set m.makerOwner = GetOwningPlayer(maker)
        set m.kind = "exchangeTerrains"
