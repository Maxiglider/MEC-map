//TESH.scrollpos=0
//TESH.alwaysfold=0
library MakeTerrainCreate needs Make, BasicFunctions, ModifyTerrainFunctions
...
struct MakeTerrainCreate extends Make    
    ...
    method getTerrainType takes nothing returns TerrainType
        return .terrainType
    endmethod
	
	static method create takes unit maker, TerrainType terrainType returns MakeTerrainCreate
        local MakeTerrainCreate m
		if (maker == null or terrainType == 0) then
			return 0
		endif
        set m = MakeTerrainCreate.allocate()
        set m.maker = maker
		set m.makerOwner = GetOwningPlayer(maker)
        set m.terrainType = terrainType
        set m.kind = "terrainCreate"
