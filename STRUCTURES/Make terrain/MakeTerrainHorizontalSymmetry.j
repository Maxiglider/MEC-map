//TESH.scrollpos=38
//TESH.alwaysfold=0
library MakeTerrainHorizontalSymmetry needs Make, BasicFunctions, ModifyTerrainFunctions



struct MakeTerrainHorizontalSymmetry extends Make    
    real lastX
    real lastY
	...
	static method create takes unit maker returns MakeTerrainHorizontalSymmetry
        local MakeTerrainHorizontalSymmetry m
		if (maker == null) then
			return 0
		endif
        set m = MakeTerrainHorizontalSymmetry.allocate()
        set m.maker = maker
		set m.makerOwner = GetOwningPlayer(maker)
        set m.kind = "terrainHorizontalSymmetry"
