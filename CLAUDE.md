...
- [docs/VISIBILITY.md](./docs/VISIBILITY.md) — what a player sees of the map: the permanent black mask, the old reveal-only rectangles kept read-only for existing maps, the per-tile system composed across levels, the minimal partition into fog modifiers, and why none of it may depend on an iteration order.
