...
export class KeyAndDoor {
    ...
    create = () => {
        ...
        this.key = CreateItem(FourCC(this.keyType.itemTypeId), this.keyX, this.keyY) ?? null
        this.opened = false
    ...
    distanceTo = (x: number, y: number) =>
        Math.min(
            ...
            SquareRoot((this.keyX - x) * (this.keyX - x) + (this.keyY - y) * (this.keyY - y))
        )
    ...
    isBetweenLocs = (x1: number, y1: number, x2: number, y2: number) => {
        ...
        return inside(r, this.doorX, this.doorY) || inside(r, this.keyX, this.keyY)
    }
    ...
    toJson = () => {
        ...
        output['keyForDoorTypeLabel'] = this.keyType.label
