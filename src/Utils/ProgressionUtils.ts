...
const initProgressionUtils = () => {
    ...
    const init = (props?: { debug?: 'level' | 'distance' | 'bfs' | 'segment'; forcedLevelIndex?: number }) => {
        ...
        const addLevelMapPoint = (levelIndex: number, x: number, y: number) => {
            ...
            if (props?.debug === 'level') {
                debugVariables.units.push(
                    Natives.UCreateUnit(Natives.UPlayer(levelIndex), String2Ascii('hfoo'), nx, ny, 0)
                )
            }
        }
            ...
            for (const [_, portalMob] of pairs(level.portalMobs.getAll())) {
                ...
                if (m1 && m2) {
                    ...
                    if (x1 && y1 && x2 && y2) {
                        ...
                        if (props?.debug === 'bfs') {
                            debugVariables.units.push(
                                Natives.UCreateUnit(Natives.UPlayer(5), String2Ascii('hfoo'), m1x, m1y, 0)
                            )
                            debugVariables.units.push(
                                Natives.UCreateUnit(Natives.UPlayer(5), String2Ascii('hfoo'), m2x, m2y, 0)
                            )
                        }
                    }
                }
        ...
        const BFS_far = (
            ...
            if (distanceMap && props?.debug === 'distance') {
                ...
                const t = Natives.UCreateTextTagUnitBJ(
                    udg_colorCode[playerId2colorId(0)] + `${0}`,
                    u,
                    -50,
                    8,
                    100,
        ...
        for (const [segmentIndex, segment] of pairs(segments)) {
            ...
            if (props?.debug === 'bfs') {
                debugVariables.units.push(
                    Natives.UCreateUnit(Natives.UPlayer(0), String2Ascii('hfoo'), segment[0].x, segment[0].y, 0)
                )
                debugVariables.units.push(
            ...
            if (foundLevelIndex !== -1) {
                ...
                for (const tile of mergedSegment) {
                    ...
                    if (props?.debug === 'segment') {
                        const u = Natives.UCreateUnit(
                            Natives.UPlayer(foundLevelIndex),
                            String2Ascii('hfoo'),
                            tile.x,
                            tile.y,
                            0
