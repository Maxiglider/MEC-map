...
- `capturedTerrain: TerrainType[]` — a **flat, single-dimension, row-major array** of live `TerrainType` object references (not `terrainTypeId` numbers — the runtime representation holds real references; conversion to/from a serializable form only happens at the `toJson()`/`newFromJson()` boundary). Indexed as `capturedTerrain[row * width + col]`, alongside `originX/originY/width/height` (tile-index units, converted via `Constants.LARGEUR_CASE`), bounds sourced from `zone.getMinX()/getMaxX()/getMinY()/getMaxY()` or `globals.MAP_MIN_X/MAX_X/MIN_Y/MAX_Y` for whole-map. There is no `null` entry case: capture fails hard (see below) rather than storing a hole. **The serialized form is the ASCII string form (`Ascii2String`), not the raw numeric `terrainTypeId`** — matching `TerrainType.toJson()`'s own convention exactly, and reconstructed the same way `TerrainTypeArray.newFromJson()` does, via `TerrainTypeMax.TerrainTypeAsciiString2TerrainTypeId()` (this engine's terrain-id catalog goes through virtualization tables in `Terrain_type_max.ts`, so a raw numeric id captured before an `-lmfc` reload isn't guaranteed to still resolve afterward — found via an actual repro: `-smic` then `-lmfc` threw `unknown terrainTypeId "<n>"` until this was fixed).
...
- **`displayTerrainSave` (`dts`) `[<label>|<levelNum>|global|g|current|c] [page]`**:
  - No param: show **every terrain save**, regardless of level/global, paginated (reuse `handlePaginationArgs`/`handlePaginationObj`, same convention as `displayMonsterSpawns` — a unified pagination overhaul across the whole command system is explicitly out of scope/future work).
  ...
  - A label (with optional `x-` prefix): detailed view — label, level (or "global"), whole-map or zone surface via `MECRegion.getSurface()`/`getSurfacePercent()` (new concrete methods on the base class, default `0`; overridden on `HorizontalRectangleRegion` with the exact tile-grid count matching `TerrainSave.captureTerrain()`'s own width/height formula — e.g. `"324 tiles (5.3%)"`), applied state, event count; move camera to zone center via `zone.getCenterX()/getCenterY()` (confirmed to exist, `MECRegion.ts:275,277` abstract, implemented up the `TrapezeRegion`→...→`HorizontalRectangleRegion` chain) unless whole-map; show `zone.debugRects(true)` timed like `MonsterSpawn.displayForPlayer`.
...
3. **Zone-scoped saves**: `MakeTerrainSaveZone`/`MakeUpdateTerrainSaveZone` click flows (`src/core/05_MAKE_STRUCTURES/Make_terrain_save/`, started via new `EscaperMake.makeSaveTerrainZone`/`makeUpdateTerrainSaveZone` methods, mirroring `makeCreateMonsterSpawn`/`makeSetMonsterSpawnZone`), `updateTerrainSave`, `displayTerrainSave` (list/detail/camera/debugRects/pagination). Both `Make*` classes share `MakeTerrainSaveZoneBase`: it lets `MakeMECRegion`'s built-in click flow run unmodified (so the landmark cross shown while clicking stays exactly under the cursor), but discards the `HorizontalRectangleRegion`/`LineRegion` it produces from the raw clicks and rebuilds the zone itself from the two raw click points, snapped to the tile grid.
   - **Tile-grid convention** (important, easy to get wrong): `MAP_MIN_X`/`MAP_MIN_Y` and every other tile-grid sample point already used across this codebase (`Change_all_terrains.ts`'s walk, `Save_terrain.ts`'s `centerOffsetX/Y`, `TerrainSave.captureTerrain()`'s own sampling) refer to a tile's **center**, not its corner. `MakeTerrainSaveZoneBase` snaps each raw click to its containing tile's center (`tileCenter()`), then moves *out* by half a tile (`Constants.LARGEUR_CASE / 2`) on each side to get the zone's actual edges — this is what makes the zone's `HorizontalRectangleRegion` (and therefore its `debugRects()` outline) exactly edge the touched tiles, e.g. clicking twice in the same tile yields exactly one 128x128 tile. Since the zone's `minX/maxX/minY/maxY` are now edges (not centers), `TerrainSave.getBounds()` converts them back to center coordinates (shifts inward by half a tile) before running its capture loop, so that loop — and the whole-map branch, `MAP_MIN_X`/`MAP_MAX_X`, both untouched — keep using the same center-based convention throughout. `HorizontalRectangleRegion.getSurface()` was updated to match (exact `(maxX-minX)/LARGEUR_CASE` tile count, no `+1` fencepost, since edge-to-edge span is already an exact multiple of the tile size).
   - This edge-snapping also guarantees the zone is always at least one full tile, so the `LineRegion` too-thin fallback can never actually trigger for this feature. `TerrainSave.setZone()` only swaps the reference (doesn't destroy the old zone) so the update flow can revert cleanly if `captureTerrain()` fails on the newly drawn zone.
...
6. **Polish — debugRect cross marker** (last step, cosmetic only): when `displayTerrainSave <label>`'s detail view shows the zone's `debugRects(true)` (see `displayTerrainSaveDetail`, `Command_functions-terrainSaves.ts`), also draw a cross at the zone's center to make the rectangle's center point visually obvious, not just its outline. This must be opt-in via a new parameter (default off) on the debugRect-drawing call, not the default behavior for every `debugRects(true)` caller elsewhere in the codebase — for now `displayTerrainSaveDetail` would be the only call site passing it.

# Critical files

- `src/core/04_STRUCTURES/TerrainSave/` (new: `TerrainSave.ts`, `TerrainSaveArray.ts`, `TerrainSaveEvent.ts`, `TerrainSaveEventArray.ts`)
- `src/core/05_MAKE_STRUCTURES/Make_terrain_save/` (new: `MakeTerrainSaveZone.ts`, `MakeUpdateTerrainSaveZone.ts`, `MakeSelectMonsterForEvent.ts`)
- `src/core/06_COMMANDS/Helpers/Command_functions-terrainSaves.ts` (new — every helper function used by `4_make_terrain_saves.ts` lives here, not in the command file itself: `isNewTerrainSaveLabelValid`, `formatTerrainSaveSummaryLine`, `displayTerrainSaveDetail`, and whatever the step 4/5 event commands need. Command files register commands only, no local function bodies.)
- `src/core/04_STRUCTURES/Escaper/EscaperMake.ts` (new `makeSaveTerrainZone`/`makeUpdateTerrainSaveZone` methods)
