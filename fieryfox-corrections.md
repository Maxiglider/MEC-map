...
- base hero collision size changed from 0 to 25 ; monster immolation on existing maps reduced by 25 to compensate (on maps with monster with less than 30 immolation radius, the hero collision size is kept at 0 to avoid gameplay differences)
- more variation in MEC Regions (horizontal/vertical rectangles, diagonal rectangles, lines, circles, parallelograms or trapeziums) can be used in new features
...
- command getTerrainInfo: added display of the more properties of the clicked terrain if it's a slide, death or walk terrain
...
- added command that allows spawned monsters to move to a random location on the other side of the spawn
...
- added resetRoundScores ⇒ put in a lua trigger a custom script with "MEC_core.resetRoundScores()" at end of cinematics for hero selection circles to disappear again (because of bug a where selection circles appear after a cinematic)
...
- added json library
- added new hook onBeforeHeroUsingMeteor (can be used to prevent the launch of a meteor, or to do custom triggers things at this kind of moment)
