...
- **Custom things of the old map** (user's rule, every map): its imported files and its loading screen are kept.
  ...
  - **A MEC 1 map's copies of the game's own tables are left out** (user's rule, 2026-09-21): `Units\AbilityData.slk`, `Units\CommandStrings.txt` and `Units\CommandFunc.txt`. The World Editor of the day put them in the map, they are of an old patch - the ability table is a quarter the size of the game's today - and importing one puts that old version back over the current one, MEC's own immolation abilities, which derive from `ANpi`, included. `excludeImports` in the spec leaves out any other import.
