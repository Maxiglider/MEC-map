...
- **A fog modifier lights nothing over MEC's black mask** (user's report, 2026-09-23): to light a dark level, give the *units* their sight back (`BlzSetUnitRealField(u, UNIT_RF_SIGHT_RADIUS, n)`) - the heroes' own, and the lamps' -, which is how an old map does it. Every player is allied with every other with shared vision, which is exactly why MEC takes a monster's sight away when it creates it.
