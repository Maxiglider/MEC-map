...
- **Custom things of the old map** (user's rule, every map): its imported files and its loading screen are kept.
  ...
  - **A MEC 1 map's `Units\AbilityData.slk` is left out** (user's rule, 2026-09-21): the World Editor of the day put a copy of the ability table in the map, it is one of an old patch (a quarter the size of the game's today), and importing it would put that old table back over the current one - MEC's own immolation abilities, which derive from `ANpi`, included. `excludeImports` in the spec leaves out any other import.
